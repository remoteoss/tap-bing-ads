...
def should_retry_httperror(exception):
    ...
    try:
        if (
            ...
            or isinstance(exception, ssl.SSLError)
            or isinstance(exception, suds.transport.TransportError)
            or isinstance(exception, socket.timeout)
            or type(exception) == URLError
        ):  # pylint: disable=consider-merging-isinstance,no-else-return)
            return True
        elif (
            type(exception) == Exception and exception.args[0][0] == 408
        ) or exception.code == 408:
...
def get_complex_type_elements(inherited_types, wsdl_type):
    ## inherited type
    if isinstance(
        wsdl_type.rawchildren[0].rawchildren[0], suds.xsd.sxbasic.Extension
...
def wsdl_type_to_schema(inherited_types, wsdl_type):
    ...
    elements = get_complex_type_elements(
        inherited_types, wsdl_type
    )  # Return json schema for complexType(recursive) wsdl
...
def get_type_map(client):
    ...
    if not type_map.get("KeyValueOfstringbase64Binary"):
        type_map["KeyValueOfstringbase64Binary"] = {
            "additionalProperties": False,
            "type": ["null", "object"],
            "properties": {
...
def get_stream_def(
    stream_name, schema, stream_metadata=None, pks=None, replication_keys=None
):
    ...
    if stream_metadata:
        for field in stream_metadata:
            if field.get("metadata").get("inclusion") == "automatic":
                mdata = metadata.write(
                    mdata, tuple(field.get("breadcrumb")), "inclusion", "automatic"
                )
            if field.get("metadata").get("fieldExclusions"):
                mdata = metadata.write(
                    mdata,
                    tuple(field.get("breadcrumb")),
                    "fieldExclusions",
                    field.get("metadata").get("fieldExclusions"),
                )
        ...
        for group_set in EXCLUSIONS[report_name]:
            if field in group_set["Attributes"]:
                mdata["metadata"]["fieldExclusions"] = [
                    ...
                    *[
                        ["properties", p]
                        for p in group_set["ImpressionSharePerformanceStatistics"]
                    ],
                ]
...
def test_credentials(account_ids):
    ...
    create_sdk_client(
        "CustomerManagementService", account_ids[0]
    )  # Create bingads sdk client
...
@bing_ads_error_handling
def sync_campaigns(
    client, account_id, selected_streams
    ...
    if "Campaign" in response_dict:
        ...
        if "campaigns" in selected_streams:
            ...
            singer.write_schema(
                "campaigns", get_core_schema(client, "Campaign"), ["Id"]
            )
            with metrics.record_counter("campaigns") as counter:
                singer.write_records(
                    "campaigns", filter_selected_fields_many(selected_fields, campaigns)
                )
...
async def sync_ads(client, selected_streams, ad_group_ids):
    ...
    @backoff.on_exception(
        ...
        giveup=lambda e: not should_retry_httperror(e),
        on_backoff=lambda details: LOGGER.info(
            "Retrying ad group %s after %d tries. Exception: %s",
            details["args"][0],  # ad_group_id is first arg
            details["tries"],
            details["exception"],
        ),
    )
    async def process_ad_group(ad_group_id):
        """Process a single ad group with retries."""
        ...
        if "Ad" in response_dict:
            return filter_selected_fields_many(selected_fields, response_dict["Ad"])
        return []

    async def process_batch(batch):
        """Process a batch of ad groups with error handling."""
        failed_ad_groups = []
        batch_results = []

        for ad_group_id in batch:
            try:
                ads = await process_ad_group(ad_group_id)
                batch_results.extend(ads)
            except Exception as e:
                LOGGER.error(
                    f"Failed to process ad group {ad_group_id} after all retries: {str(e)}"
                )
                failed_ad_groups.append(ad_group_id)

        return batch_results, failed_ad_groups
    ...
    with metrics.record_counter("ads") as counter:
        ...
        for batch_num, (batch_ads, failed_ad_groups) in enumerate(batch_results, 1):
            ...
            if batch_ads:
                ...
                counter.increment(len(batch_ads))
                total_ads += len(batch_ads)
    ...
    LOGGER.info(
        f"Completed ads sync - "
        f"Processed: {len(ad_group_ids)} ad groups, "
        f"Successful: {success_count}, "
        f"Failed: {len(all_failed_ad_groups)}, "
        f"Total ads synced: {total_ads}"
    )

    # If there were failures, log them for investigation
    if all_failed_ad_groups:
        LOGGER.warning(
            f"Failed ad groups: {all_failed_ad_groups}. "
            "These ad groups should be investigated and potentially resynced."
        )

    return total_ads, all_failed_ad_groups


async def sync_core_objects(account_id, selected_streams):
    ...
    if campaign_ids and ("ad_groups" in selected_streams or "ads" in selected_streams):
        ...
        if "ads" in selected_streams:
            ...
            total_ads, failed_ad_groups = await sync_ads(
                client, selected_streams, ad_group_ids
            )

            if failed_ad_groups:
                # Optionally handle failed ad groups (e.g., store for later retry)
                LOGGER.warning(
                    f"Account {account_id}: {len(failed_ad_groups)} ad groups failed to sync. "
                    "Consider implementing a retry mechanism for failed groups."
                )
...
async def poll_report(
    ...
    with metrics.job_timer("generate_report"):
        for i in range(1, MAX_NUM_REPORT_POLLS + 1):
            LOGGER.info(
                "Polling report job %s/%s - %s - from %s to %s",
                i,
            ...
            if response.Status == "Success":
                if response.ReportDownloadUrl:
                    ...
                    LOGGER.info(
                        "No results for report: %s - from %s to %s",
                        report_name,
                        start_date,
                        end_date,
                    )
            ...
            if i == MAX_NUM_REPORT_POLLS:
                LOGGER.info(
                    "Generating report timed out: %s - from %s to %s",
                    report_name,
                    start_date,
                    end_date,
                )
...
def stream_report(stream_name, report_name, url, report_time):
    ...
    with metrics.http_request_timer("download_report"):
        ...
        response = SESSION.get(
            url, headers={"User-Agent": get_user_agent()}, timeout=request_timeout
        )
        ...
        with zip_file.open(zip_file.namelist()[0]) as binary_file:
            with io.TextIOWrapper(binary_file, encoding="utf-8") as csv_file:
                ...
                reader = csv.DictReader(
                    (line.replace("\0", "") for line in csv_file), fieldnames=headers
                )
...
def get_report_interval(state_key):
    ...
    conversion_min_date = (
        arrow.get().floor("day").shift(days=conversion_window)
    ...
    start_date = min(
        start_date, conversion_min_date
    )  # minimum of start_date or conversion_min_date

    end_date = min(
        config_end_date, arrow.get().floor("day")
    )  # minimum of end_date or current date
...
async def sync_report(client, account_id, report_stream):
    ...
    LOGGER.info(
        "Generating %s reports for account %s between %s - %s",
...
async def sync_report_interval(client, account_id, report_stream, start_date, end_date):
    ...
    if success and download_url:  # pylint: disable=no-else-return
        LOGGER.info(
            "Streaming report: %s for account %s - from %s to %s",
            report_name,
            account_id,
            start_date,
            end_date,
        ...
        LOGGER.info(
            "No data for report: %s for account %s - from %s to %s",
            report_name,
            account_id,
            start_date,
            end_date,
...
def get_report_request_id(
    ...
    if not force_refresh and saved_request_id is not None:
        LOGGER.info("Resuming polling for account %s: %s", account_id, report_name)
...
def build_report_request(
    ...
    LOGGER.info(
        "Syncing report for account %s: %s - from %s to %s",
        account_id,
        report_name,
        start_date,
        end_date,
...
async def sync_account_data(account_id, catalog, selected_streams):
    ...
    if len(all_core_streams & set(selected_streams)):
        ...
        await sync_core_objects(account_id, selected_streams)
